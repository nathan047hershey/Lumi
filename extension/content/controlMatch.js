 ...
 * Loaded before fillShared.js, fill.js, and bidderFill.js (window.__lumiControlMatch).
 *
 * Rules:
...
(function () {
    ...
    api.isVisibleNode = (n) => {
        ...
        return n.getClientRects().length > 0 || n.offsetParent !== null;
    };

    /** Listbox/menu tied to a combobox via aria-controls / aria-owns / single open menu. */
    api.resolveListboxForAnchor = (anchorEl) => {
        if (!anchorEl) return null;
        const listId = String(anchorEl.getAttribute?.('aria-controls') || '').trim();
        if (listId) {
            const owned = document.getElementById(listId);
            if (owned) return owned;
        }
        const group = anchorEl.closest?.('[aria-owns]');
        if (group) {
            for (const id of String(group.getAttribute('aria-owns') || '').split(/\s+/)) {
                if (!id) continue;
        ...
        if (shell) {
            const local = shell.querySelector?.('[role="listbox"], .select__menu, [class*="select__menu"]');
            if (local && api.isVisibleNode(local)) return local;
        }
    ...
    /** Close other open combobox menus so option lists do not bleed together (Greenhouse EEO). */
    api.closeOtherComboboxMenus = (activeEl) => {
        const activeCombo = activeEl?.closest?.('[role="combobox"]') || activeEl;
    ...
    api.isDialCodeOptionText = (t) => {
        ...
        if (/^[A-Za-zÀ-ÿ].*\+\d{1,4}\s*$/.test(text)) return true;
        if (/\+\d{1,4}\s*$/.test(text)
            && /\b(flag|afghanistan|albania|algeria|american samoa|andorra|angola|anguilla|united states)\b/i.test(text)) {
            return true;
        }
        return false;
    };

    api.isSelectableOptionText = (t) => {
        const text = String(t || '').replace(/\s+/g, ' ').trim();
        if (!text || api.isDialCodeOptionText(text)) return false;
        if (/^no options$/i.test(text) || /^no results/i.test(text) || /^loading/i.test(text)) return false;
        if (/^type to search|^start typing|^search\.\.\./i.test(text)) return false;
        return true;
    };

    api.optionBelongsToAnchor = (node, anchorEl, scopedMenu = null) => {
        ...
        if (!api.isVisibleNode(node)) return false;
        if (!api.isSelectableOptionText(node.textContent || '')) return false;
        if (!anchorEl) return true;

        const menu = scopedMenu || api.resolveListboxForAnchor(anchorEl);
        if (menu) return menu.contains(node);

        const shell = anchorEl.closest?.(
            '.select-shell, [class*="select-shell"], .select__control, [class*="select__control"]'
        );
        if (shell && shell.contains(node)) return true;
    ...
    api.listScopedSelectOptions = (anchorEl = null) => {
        const scopedMenu = anchorEl ? api.resolveListboxForAnchor(anchorEl) : null;
        const selector = [
            '[role="option"]',
            '.select__option',
            '[id*="option-"]',
        ...
        return [...document.querySelectorAll(selector)].filter((n) =>
            api.optionBelongsToAnchor(n, anchorEl, scopedMenu)
        );
    };
