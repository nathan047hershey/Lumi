...
(() => {
    ...
    function personalValue(kind, profile, field = null) {
        ...
        switch (kind) {
            ...
            case 'requires_sponsorship': {
                return profile?.requires_sponsorship || 'No';
            }
    ...
    function valuesRoughlyMatch(wanted, got, kind = '') {
        ...
        // Gender matching - Man ≈ Male, Woman ≈ Female
        if (kind === 'gender') {
            if (/^n/.test(a) && /^n/.test(b)) return true; // Decline to answer
            const gMale = (s) => /^(man|male|m)$/i.test(s);
            const gFemale = (s) => /^(woman|female|f)$/i.test(s);
            if (gMale(a) && gMale(b)) return true;
            if (gFemale(a) && gFemale(b)) return true;
            if (gMale(a) && gMale(b)) return true;
            if (a === b) return true;
        }
        // Race/ethnicity matching - handle common EEOC option name variations
        if (kind === 'race_ethnicity') {
            if (a === b) return true;
            // Normalize common variations
            const normalizeRace = (s) => s.toLowerCase().replace(/\s+/g, ' ').trim();
            const normA = normalizeRace(a);
            const normB = normalizeRace(b);
            ...
            const raceNorm = {
                ...
                'decline to identify': 'prefer not to say'
            };
            const normA2 = raceNorm[normA] || normA;
            const normB2 = raceNorm[normB] || normB;
    ...
    function scoreOptionText(text, aliasList, kind = '') {
        ...
        // Gender matching - handle Man/Male, Woman/Female synonymy
        if (kind === 'gender') {
            const want = aliasList.join(' ').toLowerCase();
            // Man ≈ Male, Woman ≈ Female
            if (/man|male|\bm\b/.test(want) && /man|male|\bm\b/.test(t)) {
                best = Math.max(best, 90);
            }
            if (/woman|female|\bf\b/.test(want) && /woman|female|\bf\b/.test(t)) {
                best = Math.max(best, 90);
            }
            // Exact match is best
            if (t === want || t === aliasList[0]?.toLowerCase()) {
                best = Math.max(best, 100);
            }
        }
        // Race/ethnicity matching - handle common EEOC option names
        if (kind === 'race_ethnicity') {
            const want = aliasList.join(' ').toLowerCase();
            const raceMap = {
                'black or african american': ['black', 'african american', 'black or african american', 'afro'],
                'white': ['white', 'caucasian', 'white (non-hispanic)'],
                'asian': ['asian', 'asian american', 'asian or pacific islander'],
                'asian american': ['asian', 'asian american', 'asian or pacific islander'],
                'hispanic or latino': ['hispanic', 'latino', 'hispanic or latino', 'latina', 'latinx'],
                'latino': ['hispanic', 'latino', 'hispanic or latino', 'latina', 'latinx'],
                'two or more races': ['two or more races', 'multiracial', 'mixed', 'two or more'],
                'american indian or alaska native': ['american indian', 'alaska native', 'native american', 'first nation'],
                'native hawaiian or other pacific islander': ['pacific islander', 'native hawaiian', 'hawaiian'],
            ...
            // Exact text match
            if (aliasList.some((a) => t === a.toLowerCase())) {
                best = Math.max(best, 100);
            }
        }
        return best;
    ...
    async function fillComboboxSimplify(el, preferred, {
        ...
        const verified = () => {
            ...
            if (kind === 'gender') {
                ...
                const w = wantRaw.toLowerCase();
                // Man ≈ Male, Woman ≈ Female
                if (/^(man|male|m)$/i.test(w) && /^(man|male|m)$/i.test(g)) return true;
                if (/^(woman|female|f)$/i.test(w) && /^(woman|female|f)$/i.test(g)) return true;
                if (w === g) return true;
            }
            // Race/ethnicity matching: handle common EEOC option name variations
            if (kind === 'race_ethnicity') {
                ...
                const raceNorm = {
                    ...
                    'latina': 'hispanic or latino', 'latinx': 'hispanic or latino',
                    'multiracial': 'two or more races', 'mixed': 'two or more races',
                    'native american': 'american indian or alaska native',
                    'american indian': 'american indian or alaska native',
                    'pacific islander': 'native hawaiian or other pacific islander',
                    'decline': 'prefer not to say', 'prefer not to say': 'prefer not to say'
                };
                const normW = raceNorm[w] || w;
                const normG = raceNorm[g] || g;
                if (normW === normG) return true;
            }
    ...
    /** In-page gap fill — used by bidder Greenhouse engine instead of background FILL_FORM top-up. */
    async function topUpMissingFields(payload = {}) {
        const form = collectForm();
        if (form.blocked) {
            return {
                filled: 0,
        ...
        const fillStats = await fillForm({
            ...
            jobRole: payload.jobRole || payload.job_role || '',
            skipQuestions: false
        });
        const readiness = evaluateSubmitReadiness(form, fillStats, { uploaded: 0 });
        return {
