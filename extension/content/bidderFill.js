...
(function () {
    ...
    /** Visa sponsorship: from profile; default No. */
    ...
    function listMenuOptions(anchorEl = null) {
        const helper = cm();
    ...
    async function waitForMenuOptions(timeoutMs = 8000, anchorEl = null) {
        ...
        while (Date.now() - start < timeoutMs) {
            ...
            last = listMenuOptions(anchorEl);
            // Complete → stop waiting
            if (!loading && last.length > 0) {
                await sleep(50);
                const settled = listMenuOptions(anchorEl);
    ...
    async function fillCombobox(el, wanted, kind, strategy, profile = null, label = '', opts = {}) {
        ...
        try { helper?.closeOtherComboboxMenus?.(input); } catch (_) { /* ignore */ }
        ...
        if (yn || forceOpenOnly || /disabilit|veteran|gender|race|ethnicity|eeo|sponsor/i.test(kind + raw)) {
            options = await waitForMenuOptions(yn ? 900 : (forceOpenOnly ? 1400 : 700), input);
        ...
        if (!pick && (!yn || typeFilter) && !looksLikeEssay) {
            ...
            if (!pick) {
                options = await waitForMenuOptions(isCity ? 6000 : 5000, input);
                best = null;
                bestScore = -1;
                catchAll = null;
                for (const o of options) {
        ...
        if (!pick && forceOpenOnly && (kind === 'skill_experience' || /^education_(start|end)_/.test(kind))) {
            ...
            options = await waitForMenuOptions(2500, input);
            best = null;
            bestScore = -1;
            for (const o of options) {
                const t = (o.textContent || '').replace(/\s+/g, ' ').trim();
    ...
    async function runEngine(payload) {
        ...
        if (!requiredComplete && typeof window.__lumiFillEngine?.topUpMissingFields === 'function') {
            try {
                const topUp = await window.__lumiFillEngine.topUpMissingFields({
                    profile,
                    answers,
                ...
                topUpFilled = topUp?.filled || 0;
                if (topUpFilled > 0) {
                    filled += topUpFilled;
                    attempts.push({
                        field: '__topup__',
                        adapter: topUp?.adapter || 'autofill-engine-v3',
                        strategy: topUp?.strategy || 'topup',
                        ok: true,
                        filled: topUpFilled
                    });
                }
    ...
    try {
        window.__lumiBidderEngine = { ENGINE, detectAts, runEngine };
    } catch (_) { /* ignore */ }
}());
