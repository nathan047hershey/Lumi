...
export default function AutoBidderDialog({ open, onOpenChange, isAdmin, selectedLinks = [], panelToken = 0 }) {
    ...
    const [lumiVersion, setLumiVersion] = useState(() => getLumiBridgeVersion());
    const [lumiOk, setLumiOk] = useState(() => {
        const v = getLumiBridgeVersion();
        return v ? versionAtLeast(v, MIN_LUMI_VERSION) : null;
    });
    const [loading, setLoading] = useState(false);
    ...
    const [packetStatus, setPacketStatus] = useState({ ready: false, total: 0, items: [] });
    const requirePacketBeforeProcess = lumiPrefs.requirePacketBeforeProcess !== false;
    ...
    const pingLumi = useCallback(async () => {
        ...
        try {
            ...
            setLumiOk(stale ? versionAtLeast(stale, MIN_LUMI_VERSION) : false);
            return stale;
        }
    }, []);
    ...
    useEffect(() => {
        ...
        loadReadyPreview();
    }, [open, monitorActive, loadReadyPreview, profileId]);
    ...
    const openApplyTabsWithoutExtension = async () => {
        ...
        try {
            ...
            setStatus(
                opened
                    ? `Opened ${opened} apply tab(s). Use Web Fill bookmarklet (Setup) or install Lumi v${MIN_LUMI_VERSION}+ for Auto Bid.`
    ...
    return (
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="flex h-[min(96vh,60rem)] max-h-[96vh] w-[min(98vw,80rem)] max-w-7xl flex-col gap-0 overflow-hidden border-white/[0.08] bg-[hsl(240_6%_9%)] p-0 shadow-[0_24px_80px_-12px_rgba(0,0,0,0.7),0_0_0_1px_hsla(187,85%,53%,0.1)] sm:rounded-2xl">
                ...
                {(queueStatusLabel || bidProgress.pct > 0 || (courseStats.success + courseStats.failed + courseStats.filled + courseStats.attention + courseStats.running) > 0) && (
                    <div className="shrink-0 rounded-xl border border-white/[0.08] bg-black/25 px-3.5 py-2.5">
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
                            {queueStatusLabel ? (
                                <p className="min-w-0 flex-1 text-sm font-medium text-white/90">{queueStatusLabel}</p>
                            ) : null}
                            <div className="flex flex-wrap gap-1.5 text-[10px]">
                                {courseStats.success > 0 && (
                                    <Badge className="border-emerald-500/40 bg-emerald-600/80 text-[10px] font-semibold">{courseStats.success} OK</Badge>
                                )}
                                {courseStats.failed > 0 && (
                                    <Badge className="border-red-500/40 bg-red-600/80 text-[10px] font-semibold">{courseStats.failed} FAIL</Badge>
                                )}
                                {courseStats.filled > 0 && (
                                    <Badge className="border-cyan-500/40 bg-cyan-600/70 text-[10px] font-semibold">{courseStats.filled} FILLED</Badge>
                                )}
                                {courseStats.attention > 0 && (
                                    <Badge className="border-amber-500/40 bg-amber-600/80 text-[10px] font-semibold">{courseStats.attention} CAPTCHA</Badge>
                                )}
                                {courseStats.running > 0 && (
                                    <Badge variant="secondary" className="text-[10px] font-semibold">{courseStats.running} RUN</Badge>
                                )}
                            </div>
                        </div>
                        ...
                        {(queueState?.queueStartedAt || detail?.course?.started_at) ? (
                            <p className="mt-1.5 text-[10px] tabular-nums text-white/45">
                                {detail?.course?.started_at || queueState?.jobStartedAt ? (
                                    <>Job <span className="font-mono text-cyan-300/90">{formatTotalElapsed(detail?.course?.started_at || queueState?.jobStartedAt, detail?.course?.applied_at || detail?.course?.filled_at || null)}</span></>
                                ) : null}
                                {(detail?.course?.started_at || queueState?.jobStartedAt) && queueState?.queueStartedAt ? ' · ' : null}
                                {queueState?.queueStartedAt ? (
                                    <>Queue <span className="font-mono text-cyan-300/90">{formatTotalElapsed(queueState.queueStartedAt, queueState.queueEndedAt || (/^(?:done|stopped|empty)$/i.test(String(queueState?.status || '')) ? queueState.updatedAt : null))}</span></>
                                ) : null}
                                {Number(queueState?.total) > 0 && Number(queueState?.index) > 0 ? ` · ${queueState.index}/${queueState.total}` : null}
                            </p>
                        ) : null}
                    </div>
                )}
                    ...
                    >
                        ...
                        <ModalTabsContent value="setup" className="flex min-h-0 flex-1 flex-col overflow-hidden">
                            <AutoBidderSetupPanel
                                ...
                                matchedProfileOptions={matchedProfileOptions}
                                otherProfileOptions={otherProfileOptions}
                                ...
                                requirePacketBeforeProcess={requirePacketBeforeProcess}
                                onPacketStatusChange={setPacketStatus}
                                selectedLinks={selectedLinks}
                                unsupportedSelected={unsupportedSelected}
                                linkOutcomeForProfile={linkOutcomeForProfile}
                                primaryJobUrl={primaryJobUrl}
                                profiles={profiles}
                                lumiOk={lumiOk}
                                ...
                                onCheckLumi={async () => {
                                    ...
                                    try { await reinjectLumiBridge(8000); } catch (_) { /* ping */ }
                                    const ver = await pingLumi();
                                    if (versionAtLeast(ver, MIN_LUMI_VERSION)) {
                                        ...
                                        setStatus('');
                                        setError(
                                            `Lumi not connected (need v${MIN_LUMI_VERSION}+). Open chrome://extensions → Reload Lumi, then Check again.`
                                        );
                                    }
                                }}
                                ...
                                lumiPrefsAutoNext={!!lumiPrefs.autoNext}
                                memoryBusy={memoryBusy}
                                refreshStudyingPanel={refreshStudyingPanel}
                                helperProbe={helperProbe}
                                questionMemoryRows={questionMemoryRows}
                                onToggleQuestionMemory={async (row) => {
                                    setMemoryBusy(true);
                                    try {
                                ...
                                clearHostInput={clearHostInput}
                                setClearHostInput={setClearHostInput}
                                onClearHostLessons={async () => {
                                ...
                                outlookBusy={outlookBusy}
                                outlookDevice={outlookDevice}
                                outlookMsg={outlookMsg}
                                connectOutlookGraph={connectOutlookGraph}
                                disconnectOutlookGraph={disconnectOutlookGraph}
                                removeOutlookMailbox={removeOutlookMailbox}
                                subscribeOutlookPush={async (mailboxId) => {
                                    setOutlookBusy(true);
                                    try {
                                        await userAPI.subscribeOutlookPush({ mailbox_id: mailboxId });
                                        await refreshOutlookStatus();
                                    } catch (err) {
                                ...
                                onResumeCaptcha={() => resumeCaptcha({ force: true, focusTab: true })}
                                onReAutofill={() => reAutofillCurrentJob()}
                                onStop={() => runExt('JOB_APPLY_BIDDER_STOP', 'Stop')}
                                onNext={() => runExt('JOB_APPLY_BIDDER_NEXT', 'Next')}
                                onManualFill={openApplyTabsWithoutExtension}
                                ...
                                controlInstruct={controlInstruct}
                                setControlInstruct={setControlInstruct}
                                controlInstructBusy={controlInstructBusy}
                                onApplyInstruction={async () => {
        ...
        <BidMonitorDock
            ...
            cvCourseId={detail?.course?.id || ''}
            cvHtml={detail?.application?.draft_html || ''}
            onLoadCv={async () => {
                const courseId = detail?.course?.id;
                if (!courseId) return detail?.application?.draft_html || '';
                const { data } = await coursesApi.getBidCourseCvHtml(courseId);
                return data?.html || detail?.application?.draft_html || '';
            }}
            settingsHref={isAdmin ? '/admin/bidder-settings' : '/user/bidder-settings'}
