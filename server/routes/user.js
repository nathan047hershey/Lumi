...
// ==================== BIDDER APPLICATION PACKET (pre-fill answers) ====================

router.post('/bidder/prepare-packet', async (req, res) => {
    try {
        const { application_ids, profile_id, force_regenerate } = req.body || {};
        ...
        console.error('Prepare packet error:', error.message || error);
        res.status(500).json({ error: error.message || 'Failed to prepare application packet' });
    }
});
    ...
    try {
        ...
        res.json({
            ...
            summary: {
                total: items.length,
                ready,
                not_ready: items.length - ready
            }
        });
    } catch (error) {
        res.status(500).json({ error: error.message || 'Failed to read packet status' });
    }
});

router.get('/bidder/packet/:applicationId', (req, res) => {
    try {
        ...
        if (!app) return res.status(404).json({ error: 'Application not found' });
        const packet = require('../services/bidderPacketService');
        res.json(packet.getPacketStatus(appId));
    } catch (error) {
        res.status(500).json({ error: error.message || 'Failed to load packet' });
    }
});

router.patch('/bidder/packet/:applicationId', (req, res) => {
    try {
        const appId = parseInt(req.params.applicationId, 10);
        if (!Number.isInteger(appId) || appId <= 0) {
            return res.status(400).json({ error: 'Invalid application id' });
        ...
        if (!app) return res.status(404).json({ error: 'Application not found' });
        const packet = require('../services/bidderPacketService');
        const updated = packet.updatePacketAnswer(appId, {
            index: req.body?.index,
            label: req.body?.label || req.body?.question,
            answer: req.body?.answer || req.body?.value,
            userId: req.user.id
        });
        res.json(updated);
    } catch (error) {
        res.status(500).json({ error: error.message || 'Failed to update packet answer' });
    }
});
...
router.get('/bid-courses/:id/cv-html', (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const course = userIsAdmin(req)
            ? getOne('SELECT application_id FROM bid_courses WHERE id = ?', [id])
            : getOne('SELECT application_id FROM bid_courses WHERE id = ? AND user_id = ?', [id, req.user.id]);
        if (!course) return res.status(404).json({ error: 'Course not found' });
        const app = getOne(
