...
const GEMINI_CALL_TIMEOUT_MS = parseInt(process.env.GEMINI_SCRAPE_TIMEOUT_MS || 20000, 10);
const GEMINI_MAX_ATTEMPTS = parseInt(process.env.GEMINI_SCRAPE_MAX_ATTEMPTS || 3, 10);
const GEMINI_BUDGET_MS = parseInt(process.env.GEMINI_SCRAPE_BUDGET_MS || 45000, 10);
 ...
 */
async function checkJobLinkAlive(url, timeoutMs = 10000) {
...
async function callGeminiGenerate(apiKey, model, prompt) {
    ...
    const res = await axios.post(
        ...
        { timeout: GEMINI_CALL_TIMEOUT_MS, validateStatus: () => true }
    );
    return res;
}
...
${text}`;
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (Date.now() - budgetStarted >= GEMINI_BUDGET_MS) {
        ...
        for (const model of MODEL_CANDIDATES) {
            ...
            try {
                ...
                errors.push(`slot${slot}/${model}: ${err.message || err}`);
                continue;
            }
        }
    }
